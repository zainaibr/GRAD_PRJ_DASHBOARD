...
def _safe_yellow_orange_gradient(data: pd.DataFrame):
    """Original-style dataframe heat shading without requiring matplotlib on Streamlit Cloud."""
    if data.empty:
        return data
    ...
    def _style_column(col: pd.Series):
        vals = pd.to_numeric(col, errors="coerce")
        vmin, vmax = vals.min(skipna=True), vals.max(skipna=True)
        if not np.isfinite(vmin) or not np.isfinite(vmax) or np.isclose(vmin, vmax):
            return ["" for _ in vals]

        # Approximate the original YlOrRd background_gradient cmap using fixed colors,
        # so the app keeps the old look but does not need matplotlib installed.
        start = np.array([255, 247, 188])  # pale yellow
        end = np.array([217, 95, 14])      # orange-red
        norm = ((vals - vmin) / (vmax - vmin)).clip(0, 1).fillna(0)
        styles = []
        for t in norm:
            rgb = (start * (1 - t) + end * t).astype(int)
            text_color = "#0d1117" if t < 0.65 else "#ffffff"
            styles.append(f"background-color: rgb({rgb[0]}, {rgb[1]}, {rgb[2]}); color: {text_color};")
        return styles

    return data.style.apply(_style_column, subset=numeric_cols, axis=0)
...
with tab6:
    ...
    st.dataframe(
        _safe_yellow_orange_gradient(preview_df),
