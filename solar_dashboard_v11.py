...
def apply_layout(fig, title="", height=420):
    fig.update_layout(
        **LAYOUT,
        title=dict(
            text=title,
            font=dict(color=TEXT, size=13, weight=600),
...
<style>
/* ─────────────────────────────────────────────────────────────────────────────
   Solar PV Dashboard — polished dark theme
   ───────────────────────────────────────────────────────────────────────────── */
:root {
    --bg: #080d14;
    --panel: #0f1621;
...
[data-testid="stHeader"] {
    background: linear-gradient(180deg, rgba(8,13,20,0.95) 0%, rgba(8,13,20,0.0) 100%) !important;
...
h3 { color: var(--text) !important; font-size: 1.05rem; letter-spacing: -0.01em; }
[data-testid="stCaptionContainer"],
[data-testid="stCaptionContainer"] p,
.stCaptionContainer,
.caption { color: var(--subtext) !important; opacity: 1 !important; font-size: 0.82rem; }
...
[role="option"]:hover { background-color: var(--panel-hover) !important; }
[data-baseweb="tag"] {
...
[data-testid="stSlider"] [data-baseweb="slider"] > div > div > div:nth-child(2) > div:nth-child(1) {
    background: linear-gradient(90deg, var(--accent), #4fa8f0) !important;
}
...
[role="radiogroup"] label span { color: var(--text) !important; opacity: 1 !important; }
[data-testid="stRadio"] p { color: var(--subtext) !important; font-weight: 500 !important; font-size: 0.8rem !important; letter-spacing: 0.04em !important; text-transform: uppercase !important; }
...
[data-testid="stAlert"] {
    background: linear-gradient(135deg, rgba(0,200,150,0.06) 0%, rgba(15,22,33,0.9) 100%) !important;
...
[data-testid="stMetricLabel"] { color: var(--subtext) !important; font-size: 0.78rem !important; text-transform: uppercase !important; letter-spacing: 0.05em !important; }
...
.preview-table tbody tr:nth-child(even) { background: rgba(255,255,255,0.015); }
...
with st.sidebar:
    st.markdown(
        ...
        <div style='padding:18px 4px 10px 4px'>
          <div style='font-size:1.05rem;font-weight:800;letter-spacing:-0.02em;color:#dde6f0'>
            ☀️ Solar PV Dashboard
          </div>
          <div style='font-size:0.72rem;color:#5c7188;margin-top:3px;letter-spacing:0.04em;text-transform:uppercase'>
            Amman, Jordan · 100 kW System
          </div>
        </div>
        """,
        unsafe_allow_html=True,
    ...
    st.markdown(
        ...
        "color:#00c896;padding:2px 0 8px 0'>📂 Data Source</div>",
        unsafe_allow_html=True,
    )
    uploaded_file = st.file_uploader(
        "Upload CSV",
        type=["csv"],
        help="Schema: datetime, irr_sensor_1…6, power_kw, etc.",
    ...
    st.markdown(
        "<div style='font-size:0.68rem;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;"
        "color:#00c896;padding:2px 0 8px 0'>🎛️ Filters</div>",
        unsafe_allow_html=True,
    )
...
with st.sidebar:
    ...
    st.markdown(
        "<div style='font-size:0.68rem;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;"
        "color:#00c896;padding:2px 0 8px 0'>📊 Visualisation</div>",
        unsafe_allow_html=True,
    )
    ...
    if is_synthetic:
        st.markdown(
            "<div style='background:rgba(240,120,48,0.1);border:1px solid rgba(240,120,48,0.3);"
            "border-radius:8px;padding:10px 12px;font-size:0.8rem;color:#f0a070'>"
            "⚠️ Synthetic data active.<br>Upload your CSV above for real analysis.</div>",
            unsafe_allow_html=True,
        )
    else:
        st.markdown(
            "<div style='background:rgba(0,200,150,0.08);border:1px solid rgba(0,200,150,0.25);"
            "border-radius:8px;padding:10px 12px;font-size:0.8rem;color:#00c896'>"
            "✅ Uploaded dataset loaded.</div>",
            unsafe_allow_html=True,
        )
...
st.markdown(
    ...
    <div style='padding:16px 0 6px 0'>
      <h1 style='color:#dde6f0;margin:0;font-size:clamp(1.4rem,2.5vw,2rem)'>
        ☀️ Solar PV Analytics Dashboard
      </h1>
      <p style='color:#5c7188;margin:5px 0 0 0;font-size:0.85rem;letter-spacing:0.02em'>
        {_yr_label}&nbsp;·&nbsp;100 kW Rooftop System&nbsp;·&nbsp;Amman, Jordan (31.95°N)
        &nbsp;&nbsp;<span style='color:#8fa3ba'>|</span>&nbsp;&nbsp;
        <b style='color:#00c896'>{len(df):,}</b>
        <span style='color:#5c7188'> records selected</span>
      </p>
    </div>
    """,
    unsafe_allow_html=True,
...
# Capacity factor: mean power vs. measured peak (not hardcoded 100 kW)
...
# Temperature MAE: computed on the full unfiltered dataset for consistency with model evaluation
    ...
    for col, (label, value, color, icon) in zip(kpi_cols, kpi_data[row_start:row_start + 4]):
        with col:
            card_html = (
                ...
                f"box-shadow: 0 4px 20px rgba(0,0,0,0.3), 0 0 0 0 {color}20;'>"
                f"<div style='font-size:1.1rem;margin-bottom:4px;opacity:0.7'>{icon}</div>"
...
with tab1:
    ...
    # ── Retrieve forecast data for time-series extension ──────────────────
    ...
    if _has_forecast:
        ...
        _separator = daily.index.max()   # vertical line = end of actual data
    ...
    if _has_forecast and not _f_daily.empty:
        fig.add_trace(go.Scatter(
            ...
            fillcolor=hex_to_rgba(C_ORA, 0.07),
            name="Forecast kWh",
        ))
        fig.add_trace(go.Scatter(
        ...
        fig.add_vline(
            ...
            annotation_font=dict(color=C_ORA, size=12),
    ...
    _ts_title = (
        f"Daily Energy Output with Rolling Averages — "
        f"{daily.index.min().year}–{(_f_daily.index.max().year if _has_forecast and not _f_daily.empty else daily.index.max().year)}"
        " (kWh/day)"
    )
    apply_layout(fig, _ts_title, height=400)
    ...
    with c1:
        ...
        fig2.add_trace(go.Scatter(
            ...
            marker=dict(size=7, color=C_YEL, symbol="circle"),
            name="Trend",
        ))
    ...
    with c2:
        ...
        # Extend efficiency trend with forecast (based on forecast temperature effect)
        if _has_forecast and not _f_daily.empty:
            # Estimate forecast efficiency using eta_ref and temp correction
            _eta_ref = df_raw.attrs.get("eta_ref_used", df_raw["eta_ref_used"].median() if "eta_ref_used" in df_raw.columns else 0.185)
            _gamma = -0.004
            _t_ref = 25.0
            _f_eff = (_eta_ref * (1 + _gamma * (_f_daily["temp_mean"] - _t_ref)) * 100).clip(0, 25)
            _f_eff_ma = _f_eff.rolling(30, min_periods=1).mean()
            fig3.add_trace(go.Scatter(
            ...
            fig3.add_vline(
                x=_separator,
                line=dict(color=C_ORA, width=1.5, dash="dash"),
                annotation_text="  Forecast →",
    ...
    # ── Rolling volatility ─────────────────────────────────────────────────
    _spd_approx = 96  # ~15-min data → 96 steps per day
    vol_7d = df["power_kw"].rolling(_spd_approx * 7).std()
    fig4 = go.Figure()
    ...
    # Extend volatility with forecast power series
    if _has_forecast and not _forecast_future.empty and "forecast_power_kw" in _forecast_future.columns:
        _fc_vol = _forecast_future["forecast_power_kw"].rolling(_spd_approx * 7).std()
        fig4.add_trace(go.Scatter(
...
with tab5:
    ...
    if isinstance(forecast_future, pd.DataFrame) and not forecast_future.empty:
        ...
        # Peak forecast power: use daily-max p99 (robust against noise spikes)
