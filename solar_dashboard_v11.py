...
with st.sidebar:
    st.markdown(
        ...
        "<div style='font-size:1.05rem;font-weight:800;letter-spacing:-0.02em;color:#dde6f0'>☀️ Solar PV Dashboard</div>"
        "<div style='font-size:0.7rem;color:#5c7188;margin-top:3px;letter-spacing:0.04em;text-transform:uppercase'>Amman, Jordan · 100 kW System</div>"
        "</div>",
        unsafe_allow_html=True,
    )
    st.divider()
    st.markdown(
        ...
        "color:#00c896;padding:2px 0 6px 0'>📂 Data Source</div>",
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        "<div style='font-size:0.65rem;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;"
        "color:#00c896;padding:2px 0 6px 0'>🎛️ Filters</div>",
        unsafe_allow_html=True,
    )
...
with st.sidebar:
    ...
    st.markdown(
        "<div style='font-size:0.65rem;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;"
        "color:#00c896;padding:2px 0 6px 0'>📊 Visualisation</div>",
        unsafe_allow_html=True,
    )
    ...
    if is_synthetic:
        st.markdown(
            ...
            "⚠️ Synthetic data active. Upload your CSV above.</div>",
            unsafe_allow_html=True,
        )
    else:
        st.markdown(
...
st.markdown(
    ...
    <div style='padding:16px 0 6px 0'>
      ...
      <p style='color:#5c7188;margin:5px 0 0 0;font-size:0.85rem;letter-spacing:0.02em'>
        {_yr_label}&nbsp;&middot;&nbsp;100 kW Rooftop System&nbsp;&middot;&nbsp;Amman, Jordan (31.95\u00b0N)
...
# Capacity factor vs actual measured peak (not hardcoded 100 kW)
...
# Temp MAE on full unfiltered dataset for consistency with model evaluation
    ...
    for col, (label, value, color, icon) in zip(kpi_cols, kpi_data[row_start:row_start + 4]):
        with col:
            card_html = (
                ...
                f"<div style='font-size:1rem;margin-bottom:5px;opacity:0.65'>{icon}</div>"
...
with tab1:
    ...
    if _has_forecast:
        ...
        # Convert to ISO string — newer Plotly can't sum pandas Timestamps in add_vline
        _separator_str = daily.index.max().isoformat()
    ...
    _ts_title = f"Daily Energy Output with Rolling Averages — {daily.index.min().year}–{_end_yr} (kWh/day)"
    ...
    with c2:
        ...
        if _has_forecast and not _f_daily.empty:
            _eta_ref_val = float(df_raw["eta_ref_used"].median()) if "eta_ref_used" in df_raw.columns else 0.185
            _f_eff_ma = (_eta_ref_val * (1 + (-0.004) * (_f_daily["temp_mean"] - 25.0)) * 100).clip(0, 25).rolling(30, min_periods=1).mean()
            fig3.add_trace(go.Scatter(
    ...
    # ── Rolling volatility ─────────────────────────────────────────────────
    _spd_approx = 96  # 15-min data → 96 steps/day
