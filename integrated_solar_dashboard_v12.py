...
[data-testid="stHeader"] { background: rgba(13, 17, 23, 0.85) !important; }
...
/* ── file uploader: remove the bright white box ── */
[data-testid="stFileUploaderDropzone"] {
    background: linear-gradient(180deg, #111820 0%, #0d1117 100%) !important;
...
[data-baseweb="tag"] {
    background-color: rgba(0, 212, 170, 0.16) !important;
    border: 1px solid rgba(0, 212, 170, 0.38) !important;
    border-radius: 8px !important;
}
[data-baseweb="tag"] span,
[data-baseweb="tag"] svg {
    color: #7fffe6 !important;
    fill: #7fffe6 !important;
}
...
/* ── KPI cards: responsive grid instead of cramped 8-column row ── */
.kpi-grid {
    display: grid;
...
.kpi-card {
    background: linear-gradient(180deg, #181f28 0%, #141a22 100%);
    border-radius: 12px;
...
.stButton button {
    background: linear-gradient(135deg, #00d4aa 0%, #1f6feb 100%) !important;
    color: #ffffff !important;
    border: 0 !important;
    border-radius: 10px !important;
...
::-webkit-scrollbar-thumb { background: var(--grid-2); border-radius: 4px; }
::-webkit-scrollbar-thumb:hover { background: #484f58; }

/* make plotly chart backgrounds match */
.js-plotly-plot .plotly { background: transparent !important; }

/* ── raw data preview table, original dark container + YlOrRd-style cells ── */
...
# Final visual polish: subdued widget colors, cleaner controls, and more
# professional card/plot spacing. These are CSS-only overrides and do not
# affect the forecasting/yield logic.
st.markdown("""
<style>
.block-container { max-width: 1480px; padding-left: 3.2rem; padding-right: 3.2rem; }
[data-testid="stSidebar"] {
    background: linear-gradient(180deg, #151b23 0%, #10161d 100%) !important;
    box-shadow: 10px 0 30px rgba(0,0,0,0.22);
}
[data-testid="stSidebar"] h2, [data-testid="stSidebar"] h3 { letter-spacing: -0.02em; }
[data-testid="stFileUploaderDropzone"] {
    background: linear-gradient(180deg, rgba(22,27,34,.98), rgba(13,17,23,.98)) !important;
    border: 1px solid rgba(86,216,255,.18) !important;
    box-shadow: inset 0 0 0 1px rgba(255,255,255,.02), 0 10px 24px rgba(0,0,0,.16);
}
[data-testid="stFileUploaderDropzone"] button {
    background: linear-gradient(135deg, #00b894 0%, #1f6feb 100%) !important;
...
[data-baseweb="tag"] span, [data-baseweb="tag"] svg { color: #8ff7e9 !important; fill: #8ff7e9 !important; }
[data-testid="stSlider"] [role="slider"] {
    background: #00d4aa !important; border: 2px solid #0d1117 !important;
    box-shadow: 0 0 0 3px rgba(0,212,170,.20), 0 4px 10px rgba(0,0,0,.35) !important;
}
.kpi-card {
    background: radial-gradient(circle at 15% 0%, rgba(86,216,255,.07), transparent 35%),
                linear-gradient(180deg, #18212b 0%, #141a22 100%) !important;
    border-color: rgba(139,148,158,.20) !important;
    box-shadow: 0 14px 30px rgba(0,0,0,.22), inset 0 1px 0 rgba(255,255,255,.035) !important;
}
.kpi-value { font-variant-numeric: tabular-nums; }
[data-baseweb="tab-list"] { box-shadow: 0 10px 24px rgba(0,0,0,.16); }
...
FORECAST_DAYS = 365
# Reference performance from the supplied temperature LSTM notebook.
# Used for the KPI when the uploaded CSV does not already contain external
# temperature predictions; this avoids reporting over-optimistic in-sample MAE.
LSTM_TEMP_MAE_C = 0.9299
...
def _auto_predict_column(df: pd.DataFrame, target_col: str, pred_col: str, extra_cols: list[str] | None = None, floor_zero: bool = False) -> pd.DataFrame:
    ...
    if pred_col in df.columns and df[pred_col].notna().sum() >= max(10, int(0.1 * len(df))):
        ...
        df.attrs[f"{pred_col}_source"] = "uploaded"
        return df

    df.attrs[f"{pred_col}_source"] = "generated"
...
def _compute_yield_columns(df: pd.DataFrame, future: pd.DataFrame | None = None) -> tuple[pd.DataFrame, pd.DataFrame | None]:
    ...
    if future is not None and not future.empty:
        ...
        future["forecast_efficiency"] = (eta_ref * temp_factor_future * 100).clip(0, 25)
        future["forecast_power_kw"] = (future["predicted_irradiance_kw_m2"] * AREA_DEFAULT_M2 * eta_ref * temp_factor_future).clip(lower=0)
        future = _calibrate_future_power_to_history(df, future)
...
def _robust_power_peak_kw(series: pd.Series, q: float = 0.995) -> float:
...
def _estimate_display_capacity_kw(hist_df: pd.DataFrame, future_df: pd.DataFrame | None = None) -> tuple[float, str]:
    """Return the capacity denominator used only for the KPI display.

    The original dashboard label says 100 kW, but some uploaded CSV files only
    contain a ~5 kW measured channel. Using 100 kW on those files makes the
    capacity factor look artificially tiny. If the measured robust peak is far
    below 100 kW, use the dataset's robust observed/forecast peak as the display
    capacity and say so.
    """
    hist_peak = _robust_power_peak_kw(hist_df.get("power_kw", pd.Series(dtype=float)), 0.995)
    future_peak = 0.0
    if isinstance(future_df, pd.DataFrame) and "forecast_power_kw" in future_df.columns:
        future_peak = _robust_power_peak_kw(future_df["forecast_power_kw"], 0.995)
    inferred = max(hist_peak, future_peak)
    if inferred > 0 and inferred < SYSTEM_CAPACITY_KW * 0.35:
        return inferred, "estimated from the uploaded power channel"
    return SYSTEM_CAPACITY_KW, "nominal 100 kW system capacity"


def _calibrate_future_power_to_history(df: pd.DataFrame, future: pd.DataFrame) -> pd.DataFrame:
    """Keep forecast power on the same scale as the uploaded measurements.

    This prevents the 1-year power forecast from looking too small/large when
    the uploaded power channel is scaled differently from the hard-coded 100 kW
    site label. It uses robust daytime peaks rather than raw max spikes.
    """
    if future is None or future.empty or "forecast_power_kw" not in future.columns:
        return future
    future = future.copy()
    hist_day = df.loc[df.get("irradiance_avg", pd.Series(index=df.index, dtype=float)) > 200, "power_kw"] if "power_kw" in df.columns else pd.Series(dtype=float)
    hist_peak = _robust_power_peak_kw(hist_day, 0.995)
    future_day = future.loc[future.get("predicted_irradiance", pd.Series(index=future.index, dtype=float)) > 200, "forecast_power_kw"]
    future_peak = _robust_power_peak_kw(future_day, 0.995)
    if hist_peak > 0 and future_peak > 0:
        scale = float(np.clip(hist_peak / future_peak, 0.75, 1.35))
        future["forecast_power_kw"] = future["forecast_power_kw"] * scale
        cap = max(_robust_power_peak_kw(df["power_kw"], 0.999) * 1.08, hist_peak * 1.12)
        if np.isfinite(cap) and cap > 0:
            future["forecast_power_kw"] = future["forecast_power_kw"].clip(upper=cap)
    return future


def _filter_future_like_dashboard(future_df: pd.DataFrame | None, months: list[int], hours: tuple[int, int], irr_threshold: float) -> pd.DataFrame | None:
    if not isinstance(future_df, pd.DataFrame) or future_df.empty:
        return None
    out = future_df.copy()
    if months:
        out = out[out.index.month.isin(months)]
    out = out[(out.index.hour >= hours[0]) & (out.index.hour <= hours[1])]
    if irr_threshold > 0 and "predicted_irradiance" in out.columns:
        out = out[out["predicted_irradiance"] >= irr_threshold]
    return out


def _forecast_daily_summary(future_df: pd.DataFrame | None) -> pd.DataFrame:
    if not isinstance(future_df, pd.DataFrame) or future_df.empty:
        return pd.DataFrame()
    out = future_df.copy()
    if "forecast_efficiency" not in out.columns and "eta_ref_used" in out.columns and "predicted_temperature" in out.columns:
        temp_factor = (1 + GAMMA_DEFAULT * (out["predicted_temperature"] - T_REF_DEFAULT_C)).clip(lower=0)
        out["forecast_efficiency"] = (out["eta_ref_used"] * temp_factor * 100).clip(0, 25)
    agg_map = {}
    if "Predicted_Energy_Yield" in out.columns:
        agg_map["forecast_energy"] = ("Predicted_Energy_Yield", "sum")
    if "forecast_power_kw" in out.columns:
        agg_map["forecast_power_mean"] = ("forecast_power_kw", "mean")
        agg_map["forecast_power_max"] = ("forecast_power_kw", "max")
    if "predicted_irradiance" in out.columns:
        agg_map["predicted_irradiance"] = ("predicted_irradiance", "mean")
    if "predicted_temperature" in out.columns:
        agg_map["predicted_temperature"] = ("predicted_temperature", "mean")
    if "forecast_efficiency" in out.columns:
        agg_map["forecast_efficiency"] = ("forecast_efficiency", "mean")
    if not agg_map:
        return pd.DataFrame()
    return out.resample("D").agg(**agg_map).dropna(how="all")


def _add_forecast_separator(fig, forecast_start) -> None:
    """Add a forecast-start separator without Plotly's add_vline datetime bug.

    Plotly's ``fig.add_vline(..., annotation_text=...)`` can crash on newer
    Plotly/Narwhals/Pandas stacks when x is a pandas Timestamp, because its
    annotation helper tries to add Timestamp values together. A manual shape +
    annotation gives the same visual result and is stable on Streamlit Cloud.
    """
    if forecast_start is None or pd.isna(forecast_start):
        return

    try:
        x_val = pd.to_datetime(forecast_start).to_pydatetime()
    except Exception:
        x_val = forecast_start
    ...
    fig.add_shape(
        ...
        y0=0,
        y1=1,
        line=dict(color=C_ORA, width=2, dash="dash"),
    )
    fig.add_annotation(
        x=x_val,
        y=1.03,
...
def _prepare_uploaded_pipeline(raw: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if df.attrs.get("predicted_temperature_source") == "generated":
        df.attrs["temperature_model_mae_c"] = LSTM_TEMP_MAE_C
...
def load_data(uploaded=None):
    ...
    if df_syn.attrs.get("predicted_temperature_source") == "generated":
        df_syn.attrs["temperature_model_mae_c"] = LSTM_TEMP_MAE_C
...
forecast_future_full = df_raw.attrs.get("future_forecast")
forecast_future_filtered = _filter_future_like_dashboard(forecast_future_full, sel_months, sel_hours, irr_min)
forecast_start = forecast_future_full.index.min() if isinstance(forecast_future_full, pd.DataFrame) and not forecast_future_full.empty else None
forecast_end = forecast_future_full.index.max() if isinstance(forecast_future_full, pd.DataFrame) and not forecast_future_full.empty else df_raw.index.max()
year_label = f"{df_raw.index.min().year}–{max(df_raw.index.max(), forecast_end).year}"
...
st.markdown(
    ...
    <div style='padding:18px 0 4px 0'>
      ...
      <p style='color:#8b949e;margin:4px 0 0 0'>
        {year_label} · Historical data + 1-year forecast · Amman, Jordan (31.95°N) ·
...
display_capacity_kw, capacity_basis = _estimate_display_capacity_kw(df, forecast_future_filtered)
cap_factor     = (df["power_kw"].mean() / display_capacity_kw * 100) if display_capacity_kw > 0 else 0
irr_mae        = df_day["irradiance_abs_error"].mean() if not df_day.empty else 0
temp_mae       = df_raw.attrs.get("temperature_model_mae_c", df_day["temp_abs_error"].mean() if not df_day.empty else 0)
...
# Render KPI cards with Streamlit columns instead of one large Markdown/HTML block.
# This avoids Markdown interpreting indented HTML as a code block on Streamlit Cloud.
for row_start in range(0, len(kpi_data), 4):
    kpi_cols = st.columns(4)
    for col, (label, value, color) in zip(kpi_cols, kpi_data[row_start:row_start + 4]):
        with col:
            card_html = (
...
st.caption(f"Capacity factor uses {display_capacity_kw:.1f} kW ({capacity_basis}). Temperature MAE uses the supplied LSTM model metric when the CSV does not include external temperature predictions.")
...
with tab1:
    ...
    daily = df.resample("D").agg(
        power_total=("energy_kwh", "sum") if "energy_kwh" in df.columns else ("power_kw", "sum"),
        irradiance_mean=("irradiance_avg", "mean"),
        temp_mean=("avg_module_temp", "mean"),
        efficiency_mean=("efficiency", "mean"),
    ).dropna()
    future_daily = _forecast_daily_summary(forecast_future_filtered)
    ...
    if not future_daily.empty and "forecast_energy" in future_daily.columns:
        fig.add_trace(go.Scatter(
            x=future_daily.index, y=future_daily["forecast_energy"],
            mode="lines", line=dict(color=C_ORA, width=2.1, dash="dash"),
            fill="tozeroy", fillcolor="rgba(255,140,66,0.10)",
            name="Forecast daily kWh",
        ))
        _add_forecast_separator(fig, forecast_start)
    # peak annotation
    if len(daily) > 0:
    ...
    with c2:
        ...
        if not future_daily.empty and "forecast_efficiency" in future_daily.columns:
            fig3.add_trace(go.Scatter(
                x=future_daily.index, y=future_daily["forecast_efficiency"],
                mode="lines", line=dict(color=C_ORA, width=2.0, dash="dash"),
                name="Forecast efficiency",
            ))
            _add_forecast_separator(fig3, forecast_start)
        apply_layout(fig3, "Module Efficiency Trend (%)", height=340)
        st.plotly_chart(fig3, use_container_width=True)

    # ── Rolling volatility ─────────────────────────────────────────────────
    rolling_window = max(2, _steps_per_day(df.index) * 7)
    vol_7d = df["power_kw"].rolling(rolling_window).std()
    fig4 = go.Figure()
    fig4.add_trace(go.Scatter(
        x=vol_7d.index, y=vol_7d.values,
        mode="lines", line=dict(color=C_ORA, width=1.5),
        fill="tozeroy", fillcolor="rgba(255,140,66,0.15)",
        name="Historical 7-day rolling σ",
    ))
    if isinstance(forecast_future_filtered, pd.DataFrame) and not forecast_future_filtered.empty and "forecast_power_kw" in forecast_future_filtered.columns:
        f_vol = forecast_future_filtered["forecast_power_kw"].rolling(max(2, _steps_per_day(forecast_future_filtered.index) * 7)).std()
        fig4.add_trace(go.Scatter(
            x=f_vol.index, y=f_vol.values,
            mode="lines", line=dict(color=C_YEL, width=1.5, dash="dash"),
            fill="tozeroy", fillcolor="rgba(240,192,64,0.10)",
            name="Forecast 7-day rolling σ",
        ))
        _add_forecast_separator(fig4, forecast_start)
...
with tab5:
    ...
    forecast_future = forecast_future_full
    if isinstance(forecast_future, pd.DataFrame) and not forecast_future.empty:
        ...
        model_capacity_kw, _model_capacity_basis = _estimate_display_capacity_kw(df_raw, forecast_future)
        peak_forecast_power = min(float(forecast_future["forecast_power_kw"].max()), model_capacity_kw * 1.08) if model_capacity_kw > 0 else float(forecast_future["forecast_power_kw"].max())
        ...
        fc3.metric("Peak forecast power", f"{peak_forecast_power:.1f} kW")
