...
def _batch_time_model_forecast(df: pd.DataFrame, target_col: str, future_index: pd.DatetimeIndex, extra_train: pd.Series | None = None, extra_future: pd.Series | None = None, floor_zero: bool = False) -> pd.Series:
    # Fast deployable forecast for Streamlit: learns the same calendar seasonality
    # used by the LSTM notebooks, without needing unavailable saved neural weights.
...
def _compute_yield_columns(df: pd.DataFrame, future: pd.DataFrame | None = None) -> tuple[pd.DataFrame, pd.DataFrame | None]:
    ...
    df["efficiency"] = np.where(denom > 0, df["power_kw"] / denom, np.nan)
    df.loc[df["irradiance_avg"] < 50, "efficiency"] = np.nan
    df["efficiency"] = pd.Series(df["efficiency"], index=df.index).clip(0, 1)

    eta_candidates = df.loc[df["irradiance_avg"] > 200, "efficiency"].replace([np.inf, -np.inf], np.nan).dropna()
...
with tab5:
    ...
    if isinstance(forecast_future, pd.DataFrame) and not forecast_future.empty:
        ...
        fc3.metric("Peak forecast power", f"{forecast_future['forecast_power_kw'].max():.1f} kW")
        ...
        apply_layout(fig_fc, "Recursive 1-Year Forecast — Daily Irradiance and Module Temperature", height=360)
